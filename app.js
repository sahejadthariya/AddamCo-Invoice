const els = {
  ...
  pWatermark: document.getElementById('pWatermark'),
...
function bindPreview(i) {
  ...
  // Update Watermark
  const status = (i.status || 'unpaid').toLowerCase();
  els.pWatermark.textContent = status.toUpperCase();
  els.pWatermark.className = 'watermark ' + status;
...
function downloadPdf() {
  ...
  // Use window.print() for the most reliable PDF generation
  // It handles all CSS, fonts, and layouts perfectly compared to canvas-based tools
  const originalTitle = document.title;
  document.title = (data.number || 'invoice');
  window.print();
